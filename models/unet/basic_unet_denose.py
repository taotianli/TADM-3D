...
class TwoConv(nn.Sequential):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.temb_proj = torch.nn.Linear(2048,
                                         out_chns)
...
class BasicUNetDe(nn.Module):
    ...
    def __init__(
        ...
        # diffage embedding
        self.daemb = nn.Module()
        self.daemb.dense = nn.ModuleList([
            torch.nn.Linear(128,
                            512),
            torch.nn.Linear(512,
                            512),
    ...
    def forward(self, x: torch.Tensor, t, embeddings=None, image=None, metadata=None):
        """
        Args:
        ...
        diffage = metadata[:,1]
        daemb = get_timestep_embedding(diffage, 128)
        daemb = self.daemb.dense[0](daemb)
        daemb = nonlinearity(daemb)
        daemb = self.daemb.dense[1](daemb)
        ...
        temb = torch.cat([temb, aemb, daemb, cemb], dim=1)
