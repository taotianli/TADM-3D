...
import math
from unet.block.unetr_block import UnetrBasicBlockMod, UnetrUpBlockMod
...
class SwinUNETRDe(nn.Module):
    ...
    def __init__(
        ...
        self.encoder1 = UnetrBasicBlockMod(
            spatial_dims=spatial_dims,
        ...
        self.encoder2 = UnetrBasicBlockMod(
            spatial_dims=spatial_dims,
        ...
        self.encoder3 = UnetrBasicBlockMod(
            spatial_dims=spatial_dims,
        ...
        self.encoder4 = UnetrBasicBlockMod(
            spatial_dims=spatial_dims,
        ...
        #self.encoder10 = UnetrBasicBlockMod(
        self.encoder10 = UnetrBasicBlockMod(
            spatial_dims=spatial_dims,
    ...
    def forward(self, x_in, t, embeddings=None, image=None):
        temb = get_timestep_embedding(t, 128) #GUARDA https://github.com/ermongroup/ddim/blob/main/functions/denoising.py
        temb = self.temb.dense[0](temb)
        temb = nonlinearity(temb)
        temb = self.temb.dense[1](temb)
        #print("TEMB: ", temb.size()) # torch.Size([2, 512])
        #print("XXX1 ", x.size()) #torch.Size([2, 3, 96, 96, 96])

        if image is not None :
            #print(image.size()) #torch.Size([2, 4, 96, 96, 96])
            #print("X prima cat: ", x_in.size()) # torch.Size([2, 3, 96, 96, 96])
            x_in = torch.cat([image, x_in], dim=1) #FUSION CHANNEL-WISE
            #print("X dopo cat image to label: ", x_in.size()) #torch.Size([2, 7, 96, 96, 96]) 2,4,96,96,96
        ...
        enc0 = self.encoder1(x_in, temb)

        if embeddings is not None:
            enc0 += embeddings[0]
        enc1 = self.encoder2(hidden_states_out[0], temb)
        if embeddings is not None:
            enc1 += embeddings[1]
        enc2 = self.encoder3(hidden_states_out[1], temb)
        if embeddings is not None:
            enc2 += embeddings[2]
        enc3 = self.encoder4(hidden_states_out[2], temb)
        if embeddings is not None:
            enc3 += embeddings[3]
        dec4 = self.encoder10(hidden_states_out[4], temb)
        if embeddings is not None:
            dec4 += embeddings[4]

        dec3 = self.decoder5(dec4, hidden_states_out[3], temb)
        dec2 = self.decoder4(dec3, enc3, temb)
        dec1 = self.decoder3(dec2, enc2, temb)
        dec0 = self.decoder2(dec1, enc1, temb)
        out = self.decoder1(dec0, enc0, temb)
        logits = self.out(out)
        return logits
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        attn_drop: float = 0.0,
        drop_path: float = 0.0,
        act_layer: str = "GELU",#act_layer: tuple = 0.0,
        ...
        self.blocks = nn.ModuleList(
            [
                SwinTransformerBlock(
                    ...
                    use_checkpoint=use_checkpoint,
                    #act_layer= ("LeakyReLU", {"negative_slope": 0.1, "inplace": False})
                )
